...
@section('title', !empty($date) ? date('d-m-Y', strtotime($date)) . ' এর খবর' : ($query ?? 'অনুসন্ধান'))
...
<section class="pb-cat-hero">
    <div class="container-pb">
        @if(!empty($date))
            <h1 class="my-1">আর্কাইভ: “{{ date('d F, Y', strtotime($date)) }}”</h1>
            <div class="sub">
                @if($articles->count() > 0)
                    এই তারিখে সর্বোচ্চ {{ $articles->count() }}টি খবর পাওয়া গেছে
                @else
                    এই তারিখে কোনো খবর পাওয়া যায়নি
                @endif
            </div>
        @else
            @if(!empty(trim($query)))
                <h1 class="my-1">অনুসন্ধান: “{{ $query }}”</h1>
                <div class="sub">
                    @if($articles->count() > 0)
            ...
            @else
                <h1 class="my-1">অনুসন্ধান</h1>
                <div class="sub">খবর খুঁজতে উপরের সার্চ বক্সে লিখুন...</div>
            @endif
        @endif
    </div>
</section>
                ...
                <div class="row g-4" id="articles-wrapper">
                    ...
                    @empty
                        <div class="col-12 text-center py-5">
                            <div class="mb-3 text-muted" style="font-size: 3rem;"><i class="bi bi-search-heart"></i></div>
                            <p class="text-muted">দুঃখিত, কোনো খবর পাওয়া যায়নি। অন্য কিছু লিখে বা অন্য তারিখ সিলেক্ট করে চেষ্টা করুন।</p>
                        </div>
                    @endforelse
                </div>
            </div>
